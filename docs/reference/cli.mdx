...
1. Generates route registration code from your `app/` directory
2. Runs `templ generate` if `.templ` files exist
3. Starts Tailwind CSS watcher if `styles/input.css` exists
4. Starts the Go server with live reload
...
2. Builds Tailwind CSS (minified) if `styles/input.css` exists
3. Compiles Go binary with optimizations (`-ldflags "-s -w"`)
