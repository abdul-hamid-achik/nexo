...
<CardGroup cols={2}>
  ...
  <Card title="Server-Sent Events" icon="bolt" href="#server-sent-events">
...
Real-time streaming with Server-Sent Events for log streaming, progress updates, and live notifications.

### Project Structure

<FileTree>
  <Folder name="sse-example" defaultOpen>
    <Folder name="app" defaultOpen>
      <Folder name="api" defaultOpen>
        <Folder name="events">
          <File name="route.go" />
...
// GET /api/events - Stream events to client
func Get(c *fuego.Context) error {
    sse, err := c.SSE()
    if err != nil {
        return err
    ...
    sse.Send("connected", "Connection established")
    ...
    for {
        ...
        case <-ticker.C:
            ...
            if count >= 10 {
                sse.Send("complete", "Stream finished")
                return nil
            }
        }
    }
...
// GET /api/logs - Stream log file in real-time
func Get(c *fuego.Context) error {
    sse, err := c.SSE()
    if err != nil {
        return err
    ...
    logFile := c.QueryDefault("file", "/var/log/app.log")
    
    file, err := os.Open(logFile)
    if err != nil {
        sse.Send("error", "Failed to open log file")
        return nil
    }
    defer file.Close()
    ...
    for {
        ...
        sse.SendJSON("log", map[string]any{
            "line":      line,
            "timestamp": time.Now().Format(time.RFC3339),
        })
    }
...
// GET /api/progress - Stream task progress
func Get(c *fuego.Context) error {
    sse, err := c.SSE()
    if err != nil {
        return err
    ...
    taskID := c.QueryDefault("task", "default")
    ...
    sse.SendJSON("complete", map[string]any{
        "task_id": taskID,
        "status":  "success",
...
<div id="progress-bar" class="w-full bg-gray-200 rounded">
    <div id="progress" class="bg-blue-600 h-4 rounded" style="width: 0%"></div>
</div>
<div id="status"></div>
...
eventSource.addEventListener('tick', (e) => {
    ...
    document.getElementById('events').innerHTML += 
        `<div>Tick ${data.count} at ${data.timestamp}</div>`;
});
...
// Progress tracking
const progressSource = new EventSource('/api/progress?task=build-123');
...
progressSource.addEventListener('complete', (e) => {
    const data = JSON.parse(e.data);
    document.getElementById('status').textContent = data.message;
...
```html
<!-- Auto-connect and update content -->
<div hx-ext="sse" sse-connect="/api/events" sse-swap="tick">
    Waiting for events...
</div>

<!-- Multiple event types -->
<div hx-ext="sse" sse-connect="/api/logs">
    <div id="log-container" sse-swap="log" hx-swap="beforeend">
        <!-- Log lines appear here -->
    </div>
    <div sse-swap="error" class="text-red-500">
        <!-- Error messages appear here -->
    </div>
</div>
```

### SSE Method Reference

| Method | Description |
|--------|-------------|
| `c.SSE()` | Create SSE writer (sets headers, returns `*SSEWriter`) |
| `sse.Send(event, data)` | Send named event with string data |
...
| `sse.SendRetry(ms)` | Set client reconnection interval |
